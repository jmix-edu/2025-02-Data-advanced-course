...
public interface DeveloperRole extends UiMinimalRole {
    ...
    @ViewPolicy(viewIds = {"User.list", "Project.list", "Task_.list", "TimeEntry.list", "Project.detail", "Task_.detail", "TimeEntry.detail"})
    void screens();

    @EntityAttributePolicy(entityClass = Project.class, attributes = "tasks", action = EntityAttributePolicyAction.MODIFY)
    @EntityAttributePolicy(entityClass = Project.class, attributes = {"name", "startDate", "endDate", "manager", "status", "participants", "roadmap", "projectLabels", "totalEstimatedEfforts"}, action = EntityAttributePolicyAction.VIEW)
